...
const Tabledata = () => {
    const [Filter, setFilter] = useState<any>({ MigrationDate: { from: format(initialDateRange.from, 'yyyy-MM-dd'), to: format(initialDateRange.to, 'yyyy-MM-dd') } });
    ...
    const { data, isLoading } = useQuery({
        ...
        queryFn: async ({ signal }) => {
            ...
            const endpoint = `http://${ipAddress}:4004/Report?page=${pagination.pageIndex + 1}&limit=${pagination.pageSize}`;
    ...
    const totalQuery = useQuery({
        ...
        queryFn: ({ signal }) =>
            fetchData(`http://${ipAddress}:4004/Report/total`, "POST", body, signal),
        networkMode: "always",
        retry: false,
    ...
    return (
        <>
            ...
            <ResizablePanel defaultSize={20} className='text-white'> */}
                {/* <span className='flex items-center justify-center h-9 bg-red-400 font-bold '>Report Panel</span>  */}
                {/* <ReportPanel />
            </ResizablePanel> */}
        </>
    )
