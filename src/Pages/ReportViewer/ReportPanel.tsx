...
export default function ReportPanel() {
    ...
    //     // list payload
    //     if (Array.isArray(response)) return { rows: response, summary: null };
    //     if (Array.isArray(response?.data)) return { rows: response.data, summary: null };
    //     if (Array.isArray(response?.data?.data)) return { rows: response.data.data, summary: null };

    //     // summary payload (your API returns {summary:{...}} for TOTAL_MIGRATED types)
    //     if (response?.summary && typeof response.summary === "object") {
    //         return { rows: [], summary: response.summary };
    //     }
    //     if (response?.data?.summary && typeof response.data.summary === "object") {
    //         return { rows: [], summary: response.data.summary };
    //     }
    ...
    //             const ReportsColumn = DownloadPannel?.[t];

    //             const body: any = { reportType: t };

    //             // if your API expects a single date string for till-date:
    //             if (t === "TOTAL_MIGRATED_TILL_DATE") {
    //                 body.date = format(date.to, "yyyy-MM-dd"); // or date.from, as you prefer
    //             }

    //             const response = await queryClient.fetchQuery({
    //                 queryKey: ["reportDownload", t, body],
    ...
    //             const { rows, summary } = normalizePayload(response);

    //             let wsData: any[][] = [];

    //             // ---- SUMMARY SHEET (TOTAL_MIGRATED / TILL_DATE) ----
    //             if (summary) {
    //                 wsData = [
    ...
    //                 ];
    //             }
    //             // ---- LIST SHEET (TODAY_REPORT / OBJECT_LIST) ----
    //             else {
    //                 if (!Array.isArray(ReportsColumn) || ReportsColumn.length === 0) {
    //                     // still make a sheet with message (don’t fail whole export)
    //                     wsData = [["Message"], [`No columns config found for ${t}`]];
    //                 } else if (!rows.length) {
    //                     wsData = [["Message"], [`No data for ${t}`]];
    //                 } else {
    //                     const sortedColumns = [...ReportsColumn].sort(
    //                         (a: any, b: any) => (a?.Order ?? 0) - (b?.Order ?? 0)
    //                     );

    //                     const headers = sortedColumns.map((c: any) => String(c.header ?? ""));
    //                     const dataRows = rows.map((item: any) =>
    //                         sortedColumns.map((c: any) => item?.[c.dataKey])
    //                     );

    //                     wsData = [
    //                         ["Total Rows", String(rows.length), "Generated Date", today],
    //                         [],
    //                         headers,
    //                         ...dataRows,
    //                     ];
    ...
    //             (ws as any)["!cols"] = Array.from({ length: maxCols }).map((_, colIdx) => {
    //                 let w = 10;
    //                 for (const r of wsData) {
    //                     const v = r?.[colIdx];
    //                     w = Math.max(w, v == null ? 0 : String(v).length);
    //                 }
    //                 return { wch: Math.min(w + 2, 60) };
    //             });

    //             xlsx.utils.book_append_sheet(wb, ws, safeSheetName(sheetNameMap[t]));
    //         }
    ...
    //             `Migration_Multi_Report_${today}.xlsx`
    //         );
    ...
    //     //         : { type };
    //     // onGenerate?.(payload);
    //     console.log(DownloadPannel, "DownloadReport called with:", type);
    //     try {
    //         const ReportsColumn = DownloadPannel?.[type];
    ...
    //         console.log("📥 Generating report...", type, Body);
    ...
    const submit = async () => {
        try {
            ...
            // skip adding to combined workbook, because chart will be lost if we rebuild it in JS



            // ✅ NORMAL JSON reports (your existing logic)
            // const response = await queryClient.fetchQuery({
            //     queryKey: ["reportDownload", t, body],
            //     queryFn: async ({ signal }) => {
            //         const endpoint = `http://${ipAddress}:4004/Report/DownloadReport`;
